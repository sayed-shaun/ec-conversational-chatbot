...
class VectorDatabaseManager:
    """Owns the engine and every query against `faq_entries`."""
    ...
    def _bootstrap_extension(self) -> None:
    ...
    def _engine(self) -> Engine:
        self._bootstrap_extension()
        return create_engine(
            self._sqlalchemy_url(), pool_size=settings.DB_POOL_MAX_SIZE
        )
    ...
    def replace_all(self, rows: list[tuple[str, str, str, list[float]]]) -> int:
        with Session(self._engine()) as session:
    ...
    def upsert(self, rows: list[tuple[str, str, str, list[float]]]) -> int:
        if not rows:
            return 0
    ...
    def top_similar(self, query_embedding: list[float], top_k: int) -> list[dict]:
